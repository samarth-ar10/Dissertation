...
run_container() {
    ...
    read -p "Enter the port to expose on the host machine (default: 11345): " port
    port=${port:-11345}
    docker run -it --rm --name "$container_name" -p "$port":11345  -e GAZEBO_MASTER_URI=http://localhost:"$port" my_ros2_image /bin/bash -c "vncserver :1 -geometry 1024x768 -depth 16 && export DISPLAY=:1 && rviz2"
}
