...
public:
    ...
    : Node("middleware_node")
    {
        ...
        subscription_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>(
            "/middleware_node/joint_trajectory", 10,
            std::bind(&MiddlewareNode::trajectory_callback, this, std::placeholders::_1));

        // Publisher to the joint_trajectory_controller topic
        publisher_ = this->create_publisher<trajectory_msgs::msg::JointTrajectory>(
            "/joint_trajectory_controller/joint_trajectory", 10);

        RCLCPP_INFO(this->get_logger(), "MiddlewareNode has been started.");
    }
...
private:
    void trajectory_callback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
    {
        ...
        new_msg.header.stamp = this->now();  // Set the current time as the timestamp
