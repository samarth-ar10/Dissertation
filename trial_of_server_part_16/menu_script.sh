...
create_middle_man() {
    ...
    mkdir -p $COLCON_WS/src/middle_man/src
...
create_dummy_node() {
    ...
    mkdir -p $COLCON_WS/src/dummy_node/src
...
run_nodes() {
    if [ -f "$COLCON_WS/install/setup.bash" ]; then
        ...
        # Run middle_man node in the background and get its PID
        nohup ros2 run middle_man middle_man_node > middle_man_node.log 2>&1 &
        MIDDLE_MAN_PID=$!
        echo "middle_man_node is running with PID $MIDDLE_MAN_PID."
        
        # Run dummy_node in the background and get its PID
        nohup ros2 run dummy_node dummy_node > dummy_node.log 2>&1 &
        DUMMY_NODE_PID=$!
        echo "dummy_node is running with PID $DUMMY_NODE_PID."
